...
$.fn.createSlider = function (opt) {
	...
	return this.each(function () {
		...
		if (!options.repeat_slides) {
			changeTo_additional_actions.before.push(function (target) {
				if (target === 0) {
		...
		changeTo = function(target) {
			...
			while (i !== changeTo_additional_actions.before.length) {
				changeTo_additional_actions.before[i](target);
				i++;
			}
			...
			while (i !== changeTo_additional_actions.after.length) {
				changeTo_additional_actions.after[i](target);
				i++;
			}
		};
...
$( '.slider' ).createSlider({ 
	...
	caption_current_selector: '.slider-current'
});